...
def day_annul_plot(df:pd.DataFrame, column:str, **kwargs):
    ...
    - 支持自定义图表样式，标题和轴标签等。
    '''
    if ('year' not in df.columns) and ('day_of_year' not in df.columns):
...
# 2.3 风格化
    ...
    if 'month_ticks' in kwargs:
        month_ticks = kwargs['month_ticks']
    if 'month_labels' in kwargs:
        month_labels = kwargs['month_labels']
