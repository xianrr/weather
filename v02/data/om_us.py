...
db_path = os.path.join(script_dir, 'om_us.db')
...
def data_process(archive_df, forecast_df=None):
    ...
    # 积温
    groupby_df = groupby_df.assign(t=0.0)
    mask = (groupby_df['day_of_year'] > 105) & (groupby_df['temper'] >= 10)
    groupby_df.loc[mask, 't'] = groupby_df.loc[mask, 'temper']
    groupby_df.loc[:, 'degree_day'] = groupby_df.groupby(['year', 'city_code'])['t'].cumsum()
    groupby_df.drop(columns=['t'], inplace=True)
