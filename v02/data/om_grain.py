...
db_path = os.path.join(script_dir, 'om_grain.db')
...
def _save(start_date, end_date):
    ...
    merge_df = merge_df[['location_code', 'date', 'temperature_2m_mean', 'precipitation_sum']]
...
def get_forecast(start_date: str):
    ...
    merge_df = pd.merge(locations_df, response_df, on='LocationID')
    merge_df['date'] = pd.to_datetime(merge_df['date']).dt.strftime('%Y-%m-%d')
    forecast_df = merge_df[['location_code', 'date', 'temperature_2m_mean', 'precipitation_sum']]
...
def data_process(archive_df, forecast_df=None):
    ...
    concat_pd['city_code'] = concat_pd['location_code'].str[:-3]
    ...
    groupby_df['precip_sum7'] = groupby_df['precipitation_sum'].rolling(window=7, min_periods=1).sum()
    groupby_df.rename(columns={'temperature_2m_mean': 'temper'}, inplace=True)
    print(f'.. 处理后数据量：{groupby_df.shape[0]}; ')

    return groupby_df


def data_process_offset(offset, archive_df, forecast_df=None):
    ...
    groupby_df  = concat_pd.groupby(['city_code', 'date']).mean(variables).reset_index()
    # 梳理辅助列
    groupby_df = date_process.dayofyear_offset(groupby_df, offset=offset, leap_year=2, year_format=6)
    # 所需指标
    groupby_df['cum_precip'] = groupby_df.groupby(['year', 'city_code'])['precipitation_sum'].cumsum()
    groupby_df['precip_sum7'] = groupby_df['precipitation_sum'].rolling(window=7, min_periods=1).sum()
    groupby_df.rename(columns={'temperature_2m_mean': 'temper'}, inplace=True)
